...
- M14: camera or image-file nutrition-label input, bounded JPEG/PNG/WebP upload, strict browser/server validation, editable ordinary-food review, visible missing values, and confirmation before database insertion

Offline/PWA polish and final QA follow as M15–M16.
...
`public/js/ai.js` owns the browser API client, response revalidation, saved-food matching, unit/serving resolution, deterministic per-serving nutrition conversion, label-image validation, and review calculations. `public/js/voice.js` owns the short-lived browser `MediaRecorder` session and always releases its microphone track after stop, failure, or cancellation. Voice capture requires a current browser in a secure context such as HTTPS or localhost. AI text is never written directly to the DOM as HTML. Meal confirmation uses one IndexedDB transaction, while recipe confirmation passes through the ordinary recipe engine. AI-created foods and scanned labels pass through the ordinary food validator and database. Unknown label values stay blank and prevent deterministic calculation or saving when required. Unresolved foods or units prevent confirmation until the user selects or creates a saved food and serving.
